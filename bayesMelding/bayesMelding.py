...
def log_obsZs_giv_par(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, OMEGA = 1e-6):
    ...
    log_prior = log_like_gamma(log_sigma, sigma_rate, sigma_shape) + log_like_gamma(log_w, len_scal_shape, len_scal_rate) + log_like_gamma(log_obs_noi_scale, obs_noi_scale_shape, obs_noi_scale_rate) + log_like_normal(b, b_mu, b_Sigma)
...
def optimize(X_hatZs, y_hatZs, X_tildZs, y_tildZs, repeat=1, method='BFGS', rbf=True, OMEGA = 1e-6):
    ...
    while count != repeat:
        try:
            initial_theta=np.concatenate((np.log(np.random.gamma(1.2, 5., 1)), np.log(np.random.gamma(1., np.sqrt(num_par), num_par)), np.log(np.random.gamma(1.2, 1./0.6, 1)), np.random.normal(0., 1., 1)), axis=0)
    ...
    print 'parameters after optimisation is :'  + str([np.exp(np.array(res[0, :][i])[:-1]), np.array(res[0, :][i])[-1]])
