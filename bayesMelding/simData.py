...
def sim_hatTildZs_With_Plots(SEED = 1, areal_res = 20, point_res=100, sigma = 1., w = [1.], obs_noi_scale = 0.1, b= 2., num_hatZs=200, num_tildZs = 150, a_bias_poly_deg = 2):
    ...
    all_y_tildZs = np.array([fun_a_bias(np.mean(areal_coordinate[i], axis=0)) + b * np.mean(areal_Zs[i]) for i in range(len(areal_Zs))])
