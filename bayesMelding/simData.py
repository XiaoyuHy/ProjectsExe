...
def sim_hatTildZs_With_Plots(SEED = 200, phi_Zs = [0.8], gp_deltas_modelOut = True, phi_deltas_of_modelOut = [0.2], \
    sigma_Zs = 1.0, sigma_deltas_of_modelOut = 0.5, obs_noi_scale = 0.1, b= 0.6, areal_res = 20, point_res=100, num_hatZs=200, num_tildZs = 150, \
     a_bias_poly_deg = 2):
    np.random.seed(SEED)
    random.seed(SEED)
    ...
    # r.load('rfSimDataSEED' + str(SEED) + '.RData')
    ...
    numMO = np.array([50, 100, 150, 200, 250, 300, 350, 400])
    # numMO = np.array([50])
    moIncreNum=50
