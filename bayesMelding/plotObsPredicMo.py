...
def predic_gpRegression(theta, X_train, y_train, X_test, y_test, X_tildZs, y_tildZs, crossValFlag = False,  SEED=None, numMo = None, useSimData =False, grid= False, \
    ...
    if not useSimData:
        ...
        if predicMo:
            ...
            plt.ylabel('$Latitude$')
            plt.colorbar(im)
            plt.savefig("SEED120TrainObsAndAllTrainMo500_v1.eps",rasterized=True)
            plt.show()
            plt.close()
...
if __name__ == '__main__':
    ...
    if args.useSimData: 
        ...
        if args.SEED == 120:       
            input_folder = os.getcwd() + '/DataImogenFrNoGridMo/FPstart2016020612_FR_numObs_128_numMo_' + str(args.numMo) + '/seed' + str(args.SEED) + '/'     
        else:
