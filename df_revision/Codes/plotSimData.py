...
def sim_hatTildZs_With_Plots(SEED = 204, phi_Zs = [0.8], gp_deltas_modelOut = True, phi_deltas_of_modelOut = [0.2], \
	sigma_Zs = 1.0, sigma_deltas_of_modelOut = 0.5, num_tildZs = 150, delta = 1.5, obs_noi_scale = 0.5, b= 0.6, areal_res = 25,  resCoordsPerAreal =10, point_res=1000, num_hatZs=200,  \
	 a_bias_poly_deg = 2):
	np.random.seed(SEED)
	random.seed(SEED)
	lower_bound = np.array([-12., -6.5])
	...
	len_scale = phi_Zs[0]
	r.load('dataRsimulated/scale' + str(len_scale) + '/delta' + str(delta)  + '/rfSimDataSEED' + str(SEED) + '.RData')
...
if __name__ == '__main__':
	...
	sim_hatTildZs_With_Plots(SEED =args.SEED, phi_Zs = [args.lsZs], gp_deltas_modelOut = args.gpdtsMo, \
		phi_deltas_of_modelOut = [args.lsdtsMo], sigma_Zs = args.sigZs, sigma_deltas_of_modelOut = args.sigdtsMo, num_tildZs = args.numMo, delta=args.delta)
	# plot_predic_Zs_sim()
