...
def predic_gpRegression(theta, X_train, y_train, X_test, y_test, X_tildZs, y_tildZs, crossValFlag = False,  SEED=None, numMo = None, useSimData =False, grid= False, \
    predicMo = False, a_bias_poly_deg = 2, mu_obs_poly_deg =2,indivError = True, index_Xaxis =True, conditionZhat = True,  marginZtilde=False, conZhatZtilde= False, marginZhat=False, \
    ...
    tmp0 = np.repeat(1.,X_train.shape[0]).reshape(X_train.shape[0],1)
    X_train_extend = np.hstack((X_train, tmp0))
    mu_train = np.dot(X_train_extend, mu_obs_coefficients)
    ...
    tmp0 = np.repeat(1.,X_test.shape[0]).reshape(X_test.shape[0],1)
    X_test_extend = np.hstack((X_test, tmp0))
    mu_test = np.dot(X_test_extend, mu_obs_coefficients)
    ...
    if not useSimData:
        ...
        if predicMo:
            ...
            r.png('plotObsMoSEED' + str(SEED) + 'numMo' + str(numMo) +'NormElev.png', width = 960,  height= 560)
            r.par(mfrow=r.c(1,2))
            ...
            plt.savefig('figObsMosSEED' + str(SEED) + 'numMo' + str(numMo) +'NormElev.eps', rasterized=True)
            # plt.show()
            plt.close()
...
if __name__ == '__main__':
    ...
    print(y_tildZs.shape)
    
    # elevData = np.array([X_tildZs[i][:, 2][0] for i in range(len(X_tildZs))])
    # lonData = np.array([X_tildZs[i][:, 0][0] for i in range(len(X_tildZs))])
    # latData = np.array([X_tildZs[i][:, 1][0] for i in range(len(X_tildZs))])
    # plt.figure()
    # plt.scatter(elevData, y_tildZs,c='k', marker = 'o')
    # plt.xlabel('altitude/1000')
    # plt.ylabel('wind gust speed subtracted the mean of obs')
    # plt.savefig('altitude_Vs_windSpeeds_train')
    # plt.show()
    # plt.close()

    # plt.figure()
    # plt.scatter(lonData, y_tildZs,c='k', marker = 'o')
    # plt.xlabel('Longitude')
    # plt.ylabel('wind gust speed subtracted the mean of obs')
    # plt.savefig('longitude_Vs_windSpeeds_train')
    # plt.show()
    # plt.close()

    # plt.figure()
    # plt.scatter(latData, y_tildZs,c='k', marker = 'o')
    # plt.xlabel('Latitude')
    # plt.ylabel('wind gust speed subtracted the mean of obs')
    # plt.savefig('latitude_Vs_windSpeeds_train')
    # plt.show()
    # plt.close()
    # exit(-1)

    input_folder = input_folder + 'a_bias_poly_deg' + str(args.pd_abias) + '/'
    ...
    print(np.exp(mu[:-(args.pd_abias + 3 + args.pd_muObs + 2)]))
    ...
    relative_importance_of_betas = mu[-(args.pd_abias + 3 + args.pd_muObs + 2 -1):]/np.sqrt(np.diag(cov)[-(args.pd_abias + 3 + args.pd_muObs + 2 -1):])
    ...
    if args.useSimData:
        ...
        if args.predicMo:
            ...
            X_test = all_X_Zs
            X_test[:, 2] =  X_test[:, 2]/1000.
            y_test = all_y_Zs
            # plt.figure()
            # plt.scatter(X_test[:, 2], y_test,c='k', marker = 'o')
            # plt.xlabel('altitude/1000')
            # plt.ylabel('wind gust speed subtracted the mean of obs')
            # plt.savefig('altitude_Vs_windSpeeds')
            # plt.show()
            # plt.close()

            # plt.figure()
            # plt.scatter(X_test[:, 0], y_test,c='k', marker = 'o')
            # plt.xlabel('Longitude')
            # plt.ylabel('wind gust speed subtracted the mean of obs')
            # plt.savefig('longitude_Vs_windSpeeds')
            # plt.show()
            # plt.close()

            # plt.figure()
            # plt.scatter(X_test[:, 1], y_test,c='k', marker = 'o')
            # plt.xlabel('Latitude')
            # plt.ylabel('wind gust speed subtracted the mean of obs')
            # plt.savefig('latitude_Vs_windSpeeds')
            # plt.show()
            # plt.close()
            # exit(-1)
    ...
    predic_accuracy = predic_gpRegression(mu, X_train, y_train, X_test, y_test, X_tildZs, y_tildZs, args.crossValFlag, args.SEED, args.numMo, \
        args.useSimData, args.grid, args.predicMo, args.pd_abias, args.pd_muObs, args.indivError, args.index_Xaxis)
