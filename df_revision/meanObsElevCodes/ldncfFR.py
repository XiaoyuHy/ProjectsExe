...
def loadNetCdf(SEED=260, num_hatZs=128, num_tildZs = 150, crossValFlag=False, zeroMeanHatZs=True, moIncreNum=50, fold=10, cntry = 'FR', flagUseAvgMo = False):
	...
	r.load('my_dataObs_FR_elev.RData')
	data = r['dataObsFr']
	...
	X_hatZs = dataObs_within_squareAround_FR[idx, :3]

	y_hatZs = dataObs_within_squareAround_FR[idx, 3]
	...
	for idxNumMo in range(len(numMO)):
		...
		X_tildZs_tmp0 = dataMo_within_squareAround_FR[mask, :2]
		X_tildZs_tmp1 = dataMo_within_squareAround_FR[mask, 2]
		...
		X_tildZs = np.array([np.hstack((X_tildZs_tmp0[i] + cell_coords, np.repeat(X_tildZs_tmp1[i], cell_coords.shape[0]).reshape(cell_coords.shape[0], 1))) for i in range(X_tildZs_tmp0.shape[0])] + modelOutputX)
