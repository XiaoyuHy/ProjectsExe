...
def test_fun(useCluster, SEED):
	if useCluster:
		...
		input_folder = 'Data/FPstart2016020612_FR_numObs_' + str(128) + '_numMo_' + str(50) + '/seed' + str(120) + '/'
	...
	modelBias = np.array([0.6, 0.1, 2.5, 2.1, 2.6, 5.47022754 , 5.22854712, 2.5])
	...
	covAreas, grad_C_tildZs = cov_areal(X_tildZs, np.log(1.5), [np.log(0.1)], 3., np.log(1.0), gp_deltas_modelOut=True, log_phi_deltas_of_modelOut = [np.log(0.2)],  \
	 areal_res=20, OMEGA = 1e-6)
	print(covAreas.shape,len(grad_C_tildZs))
	exit(-1)
...
def log_obsZs_giv_par(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = False, withPrior= False, \
	a_bias_poly_deg = 2, mu_obs_poly_deg=2, rbf = True, OMEGA = 1e-6):
	...
	if gp_deltas_modelOut:
		...
		a_bias_coefficients = theta[(len(theta) - (a_bias_poly_deg+2 + mu_obs_poly_deg +2)):(len(theta) - (mu_obs_poly_deg +2))] # plus 2: one for intercept, one for elev_fp
		mu_obs_coefficients = theta[len(theta) - (mu_obs_poly_deg +2):] # plus 2: one for intercept, one for elev_fp
	else:
		log_sigma_Zs = theta[0] #sigma of GP function for Zs
		log_phi_Zs = theta[1:num_len_scal+1]  # length scale of GP function for Zs
		log_obs_noi_scale = theta[num_len_scal+1:num_len_scal+2]
	...
	tmp0 = np.repeat(1.,X_hatZs.shape[0]).reshape(X_hatZs.shape[0],1)
	X_hatZs_extend = np.hstack((X_hatZs, tmp0))
	mu_hatZs = np.dot(X_hatZs_extend, mu_obs_coefficients)
...
def log_obsZs_giv_par_with_grad(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = True, withPrior= False, \
	a_bias_poly_deg = 5, mu_obs_poly_deg =2, rbf = True, OMEGA = 1e-6):
	theta = np.array(theta)
	if rbf:
		num_len_scal = 1
	else:
	...
	# gradients of the jont likelihood with respect to a_bias
	if a_bias_poly_deg == 2:
		tmp = np.zeros((n_hatZs,X_hatZs.shape[1] + 1))
	else:
		tmp = np.zeros((n_hatZs, a_bias_poly_deg + 2)) #plus 2: one for the intercept, and one for the elev_fp

	# print('shape of tmp, X_tildZs_mean_extend :' + str((tmp.shape, X_tildZs_mean_extend.shape)))
	

	X_all_extend = np.vstack((tmp, X_tildZs_mean_extend))
	grads_a_bias = np.dot(X_all_extend.T, u)
	
	# gradients of the jont likelihood with respect to mu_obs
	if mu_obs_poly_deg == 2:
		tmp = np.zeros((n_tildZs, X_hatZs.shape[1] + 1))
	else:
		tmp = np.zeros((n_tildZs, mu_obs_poly_deg + 2)) # plus 2: one for the intercept, and one for the elev_fp

	# print('shape of tmp, X_hatZs_extend :' + str((tmp.shape, X_hatZs_extend.shape)))

	X_all_extend = np.vstack((X_hatZs_extend, tmp))
	grads_mu_obs_coef = np.dot(X_all_extend.T, u)

	grads_all_pars = np.concatenate((grads_par_covPars, grads_a_bias, grads_mu_obs_coef))
...
def optim_fun(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, num_par, method, bounds, repeat, seed, numMo):
	res = []
	count1 = 0
	LBFGSB_status = False
	while count1 != repeat:
		try:#find one intial value for which the optimisation works
			if gpdtsMo:
				...
				np.log(np.random.gamma(1., np.sqrt(num_par), num_par)),  np.zeros(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)), axis=0)     
			else:
				initial_theta=np.concatenate((np.log(np.random.gamma(1.2, 5., 1)), np.log(np.random.gamma(1., np.sqrt(num_par), num_par)), \
				np.log(np.random.gamma(1.2, 1./0.6, 1)), np.log(np.random.gamma(1.2, 5., 1)), np.zeros(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)), axis=0)
			...
			if useGradsFlag:
				tmp_res = minimize(fun=minus_log_obsZs_giv_par_with_grad, 
							   ...
							   args=(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, withPrior, a_bias_poly_deg, mu_obs_poly_deg),
		...
		if tmp_res['fun'] is not None: # if log pos at optimisation is not None, record the resutls, else, redo the otpmisation
			...
			if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero):
				...
				if count1 == (repeat -1):#if gradients from the all LBFGSB optimisation is NOT zero, do ONE ROUND OF  NON constraint optimisation AT THE LAST STEP
					...
					variance_log_covPars = tmp[:(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)]
					if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero) :
						...
						str([np.exp(np.array(tmp_res['x'])[:-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)]), np.array(tmp_res['x'])[-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2):]]))
...
def optim_RndStart_BFGS(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, num_par, method, bounds, repeat, seed, numMo, round):
	...
	while count != repeat:
		...
		if tmp_res['fun'] is not None: # if log pos at optimisation is not None, record the resutls, else, redo the otpmisation
			...
			logPosat_resOptim, grads_at_resOptim = log_obsZs_giv_par_with_grad(tmp_res['x'], X_hatZs, y_hatZs, X_tildZs, y_tildZs, \
				 gpdtsMo,  withPrior, a_bias_poly_deg, mu_obs_poly_deg)
			...
			if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero) :
				...
				print('parameters after optimisation with BFGS :'  + str([np.exp(np.array(tmp_res['x'])[:-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)]), \
					np.array(tmp_res['x'])[-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2):]]))
	...
	i = np.argmin(res[1,:])
	print('parameters after optimisation with BFGS is ' + str([np.exp(np.array(res[0, :][i])[:-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)]), \
		np.array(res[0, :][i])[-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2):]]))
	
	return [np.array(res[0, :][i]), np.array(res[1, :][i]), np.array(res[2, :][i]), np.array(res[3, :][i]), np.array(res[4, :][i])]

def optim_RndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, num_par, method, bounds, repeat, seed, numMo, round):
	...
	while count != repeat:
		...
		if tmp_res['fun'] is not None: # if log pos at optimisation is not None, record the resutls, else, redo the otpmisation
			...
			if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero):
				...
				print('parameters after optimisation withPrior  with LBFGSB :'  + str([np.exp(np.array(tmp_res['x'])[:-(4 + poly_deg -1)]), np.array(tmp_res['x'])[-(4 + poly_deg -1):]]))
				# print 'covariance of pars after optimisation withPrior is ' + str(withPrior) + \
				...
				if count == (repeat -1):#if gradients from the all LBFGSB optimisation is NOT zero, do ONE ROUND OF  NON constraint optimisation AT THE LAST STEP
					...
					if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero):
						...
						while repeat_another != max_repeat:
							...
							mu, log_like, cov, status, message = optim_fun(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, num_par, method, bounds, repeat_another,seed, numMo)
...
def optim_NotRndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, num_par, method, bounds, repeat, seed, numMo, round=1):
...
def optimise(X_hatZs, y_hatZs, X_tildZs, y_tildZs, withPrior, a_bias_poly_deg, mu_obs_poly_deg, gpdtsMo=False, useGradsFlag = False, repeat=3, seed =188, numMo =50, rounds =1, method='L-BFGS-B', rbf=True, OMEGA = 1e-6, \
	bounds = ((-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50))): 
	print('starting optimising when withPrior is ' + str(withPrior) + ' & gpdtsMo is ' + str(gpdtsMo) + \
		 '& useGradsFlag is ' + str(useGradsFlag)) 
	if a_bias_poly_deg == 2:
		bounds = bounds
	else:
		bounds = ((-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-50, 50), (-50, 50), (-50, 50),(-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50),(-50, 50), (-50, 50), (-50, 50), (-50, 50))
	...
	if numMo == numMo:
		...
		for round in range(rounds):
			res_tmp = optim_RndStart_BFGS(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, \
				num_par, method, bounds, repeat, seed, numMo, rounds)
		...
		print('parameters after optimisation withPrior is  :'  + str([np.exp(np.array(res[0, :][i])[:-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2)]), \
			np.array(res[0, :][i])[-(a_bias_poly_deg + 3 + mu_obs_poly_deg +2):]]))
		...
		for round in range(rounds):
			if round ==0:
				res_tmp = optim_NotRndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, \
					num_par, method, bounds, repeat,seed, numMo, rounds)
				# print 'res_tmp is ' + str(res_tmp)
				res.append(res_tmp)
			else:
				res_tmp = optim_RndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs,gpdtsMo, useGradsFlag, withPrior, a_bias_poly_deg, mu_obs_poly_deg, \
					num_par, method, bounds, repeat,seed, numMo, rounds)
				# print 'res_tmp is ' + str(res_tmp)
				res.append(res_tmp)
		res = np.array(res)
...
def gradsApprox(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = False,  withPrior= False,  \
	a_bias_poly_deg = 2, mu_obs_poly_deg=2, rbf = True, OMEGA = 1e-6, epsilon = 1e-7):
	print(a_bias_poly_deg)
	theta = np.array(theta)
	...
	for i in range(num_parameters):
		...
		J_plus[i] = log_obsZs_giv_par(thetaplus, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut,withPrior= False, \
	a_bias_poly_deg = a_bias_poly_deg, mu_obs_poly_deg=mu_obs_poly_deg) # Step 3
		...
		J_minus[i]= log_obsZs_giv_par(thetaminus, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut, withPrior= False, \
	a_bias_poly_deg = a_bias_poly_deg, mu_obs_poly_deg=mu_obs_poly_deg)# Step 3
...
def check_grads():
	...
	X_tildZs_in = open(input_folder + 'X_tildZs.pkl', 'rb')
	X_tildZs = pickle.load(X_tildZs_in) 
	for i in range(X_tildZs.shape[0]):
		X_tildZs[i][:,2] = X_tildZs[i][:,2]/1000.
	...
	areal_hatZs_in = open(input_folder + 'areal_hatZs.pkl', 'rb')
	areal_hatZs = pickle.load(areal_hatZs_in)
	# modelBias = np.array([0.6, 0.1, 2.5, 2.1, 2.6, 5.47022754 , 5.22854712, 2.5, 0.15, 3.5, 3.22854712, 1.0])
	modelBias = np.array([0.6, 0.1, 5.47022754 , 5.22854712, 2.5, 0.15, 3.5, 3.22854712, 1.0])
	initial_theta=np.concatenate((np.array([np.log(1.5), np.log(0.1), np.log(0.1), np.log(1.0),np.log(0.2)]),modelBias), axis=0)
	a_bias_poly_deg =2
	mu_obs_poly_deg = 2
	grads_approx = gradsApprox(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = True, withPrior= False, \
	a_bias_poly_deg = a_bias_poly_deg, mu_obs_poly_deg = mu_obs_poly_deg)
	_, grads_computed = log_obsZs_giv_par_with_grad(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = True, withPrior= False, \
	a_bias_poly_deg = a_bias_poly_deg, mu_obs_poly_deg = mu_obs_poly_deg)
...
if __name__ == '__main__':
	...
	p.add_argument('-pd_abias', type=int, dest='pd_abias', default=2, help='degree of the polynomial function of the additive model bias')
	p.add_argument('-pd_muObs', type=int, dest='pd_muObs', default=2, help='degree of the polynomial function of mean function of observations')
	...
	if args.useSimData:  
		...
		'_poly_deg_' + str(args.pd_abias) + '_lsZs_' + str(args.lsZs) + '_lsdtsMo_' + str(args.lsdtsMo) \
		...
		if args.usecntryFlag:
			if args.oneRepPerJob:
				...
				'/folder_' + str(args.folder) + '/SEED_' + str(args.SEED) + '_withPrior_' + str(args.withPrior) + '_poly_deg_' + str(args.pd_abias) + \
			...
			if args.oneRepPerJob:
				...
				'/SEED_' + str(args.SEED) + '_withPrior_' + str(args.withPrior) + '_poly_deg_' + str(args.pd_abias) + '_repeat' + str(args.repeat)
			else:
	...
	output_folder  = output_folder + 'a_bias_poly_deg' + str(args.pd_abias) 
	if not os.path.exists(output_folder):
		os.makedirs(output_folder)
	output_folder += '/'
	print('Output folder for the optim results is : ' + output_folder)
	...
	if args.crossValFlag:    
		mu, cov = optimise(X_train, y_train, X_tildZs, y_tildZs, args.withPrior,args.pd_abias, args.pd_muObs,args.gpdtsMo, args.useGradsFlag, args.repeat, args.SEED)
		...
		mu, cov = optimise(X_train, y_train, X_tildZs, y_tildZs, args.withPrior, args.pd_abias, args.pd_muObs, args.gpdtsMo, args.useGradsFlag, args.repeat, args.SEED, args.numMo)
