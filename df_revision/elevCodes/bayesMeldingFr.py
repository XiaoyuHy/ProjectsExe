...
def log_obsZs_giv_par(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut = False, withPrior= False, \
...
def log_obsZs_giv_par_with_grad(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut = True, withPrior= False, \
    a_bias_poly_deg = 5, rbf = True, OMEGA = 1e-6):
    theta = np.array(theta)
    if rbf:
        num_len_scal = 1
    ...
    if a_bias_poly_deg == 2:
        tmp = np.zeros((n_hatZs,X_hatZs.shape[1] + 2))
    else:
        tmp = np.zeros((n_hatZs,X_hatZs.shape[1] + 2 + a_bias_poly_deg -X_hatZs.shape[1]))
...
def minus_log_obsZs_giv_par_with_grad(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut = True, withPrior= False, \
    a_bias_poly_deg = 5, rbf = True, OMEGA = 1e-6):
    theta = np.array(theta)
    if rbf:
        num_len_scal = 1
...
def gradsApprox(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut = False,  withPrior= False,  \
    ...
    for i in range(num_parameters):
        ...
        J_plus[i] = log_obsZs_giv_par(thetaplus, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut) # Step 3
        ...
        J_minus[i]= log_obsZs_giv_par(thetaminus, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut)# Step 3
...
def minus_log_obsZs_giv_par(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gp_deltas_modelOut = False, withPrior= False, \
    a_bias_poly_deg = 2, rbf = True, OMEGA = 1e-6):
    theta = np.array(theta)
    if rbf:
        num_len_scal = 1
...
def minus_log_obsZs_giv_par_of_cov(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, withPrior, a_bias_poly_deg, modelBias, gp_deltas_modelOut = False, \
    rbf = True, OMEGA = 1e-6):
    theta = np.array(theta)
    if rbf:
        num_len_scal = 1
...
def optim_fun(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo):
    res = []
    count1 = 0
    LBFGSB_status = False
    while count1 != repeat:
        try:#find one intial value for which the optimisation works
            if gpdtsMo:
                ...
                np.log(np.random.gamma(1., np.sqrt(num_par), num_par)),  np.zeros(4 + poly_deg -1)), axis=0)     
            else:
                initial_theta=np.concatenate((np.log(np.random.gamma(1.2, 5., 1)), np.log(np.random.gamma(1., np.sqrt(num_par), num_par)), \
                np.log(np.random.gamma(1.2, 1./0.6, 1)), np.log(np.random.gamma(1.2, 5., 1)), np.zeros(4 + poly_deg -1)), axis=0)
            ...
            if useGradsFlag:
                tmp_res = minimize(fun=minus_log_obsZs_giv_par_with_grad, 
                               ...
                               args=(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, withPrior, poly_deg),
        ...
        if tmp_res['fun'] is not None: # if log pos at optimisation is not None, record the resutls, else, redo the otpmisation
            ...
            logPosat_resOptim, grads_at_resOptim = log_obsZs_giv_par_with_grad(tmp_res['x'], X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, \
            ...
            if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero):
                ...
                if count1 == (repeat -1):#if gradients from the all LBFGSB optimisation is NOT zero, do ONE ROUND OF  NON constraint optimisation AT THE LAST STEP
                    ...
                    if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero) :
                        ...
                        str([np.exp(np.array(tmp_res['x'])[:-(4 + poly_deg -1)]), np.array(tmp_res['x'])[-(4 + poly_deg -1):]]))
...
def optim_RndStart_BFGS(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo, round):
    ...
    while count != repeat:
        ...
        if tmp_res['fun'] is not None: # if log pos at optimisation is not None, record the resutls, else, redo the otpmisation
            ...
            # if gradients from the BFGS optimisation is zero, break out of the loop
            tmp = np.diag(np.array(tmp_res['hess_inv']))
            variance_log_covPars = tmp[:-(4 + poly_deg -1)]
            ...
            if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero) :
                ...
                print('parameters after optimisation with BFGS :'  + str([np.exp(np.array(tmp_res['x'])[:-(4 + poly_deg -1)]), np.array(tmp_res['x'])[-(4 + poly_deg -1):]]))
    ...
    i = np.argmin(res[1,:])
    print('parameters after optimisation with BFGS is ' + str([np.exp(np.array(res[0, :][i])[:-(4 + poly_deg -1)]), np.array(res[0, :][i])[-(4 + poly_deg -1):]]))
    
    return [np.array(res[0, :][i]), np.array(res[1, :][i]), np.array(res[2, :][i]), np.array(res[3, :][i]), np.array(res[4, :][i])]

def optim_RndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo, round):
                ...
                if count == (repeat -1):#if gradients from the all LBFGSB optimisation is NOT zero, do ONE ROUND OF  NON constraint optimisation AT THE LAST STEP
                    ...
                    if np.sum(flag_grads_equal_zero) == len(flag_grads_equal_zero):
                        ...
                        while repeat_another != max_repeat:
                            ...
                            mu, log_like, cov, status, message = optim_fun(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat_another,seed, numMo)
                            logPosat_resOptim, grads_at_resOptim = log_obsZs_giv_par_with_grad(mu, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, \
                        gpdtsMo,  withPrior, poly_deg)
...
def optim_NotRndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo, round=1):
...
def optimise(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, withPrior, poly_deg, gpdtsMo=False, useGradsFlag = False, repeat=3, seed =188, numMo =50, rounds =1, method='L-BFGS-B', rbf=True, OMEGA = 1e-6, \
    bounds = ((-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50))): 
    print('starting optimising when withPrior is ' + str(withPrior) + ' & gpdtsMo is ' + str(gpdtsMo) + \
         '& useGradsFlag is ' + str(useGradsFlag)) 
    if poly_deg == 2:
        bounds = bounds
    else:
        bounds = ((-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-2.3, 4.6), (-50, 50), (-50, 50), (-50, 50),(-50, 50), (-50, 50), (-50, 50), (-50, 50), (-50, 50))
    ...
    if numMo == numMo:
        ...
        for round in range(rounds):
            res_tmp = optim_RndStart_BFGS(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo, rounds)
        ...
        print('parameters after optimisation withPrior is  :'  + str([np.exp(np.array(res[0, :][i])[:-(4 + poly_deg -1)]), np.array(res[0, :][i])[-(4 + poly_deg -1):]]))
        ...
        for round in range(rounds):
            if round ==0:
                res_tmp = optim_NotRndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat,seed, numMo, rounds)
                # print 'res_tmp is ' + str(res_tmp)
                res.append(res_tmp)
            else:
                res_tmp = optim_RndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat,seed, numMo, rounds)
                # print 'res_tmp is ' + str(res_tmp)
                res.append(res_tmp)
        res = np.array(res)
        res = res.T
...
def check_grads():
    ...
    modelBias = np.array([0.6, 1.97191694, 2.5, 2.1, 2.6, 5.47022754 , 5.22854712, 2.5])
    initial_theta=np.concatenate((np.array([np.log(1.5), np.log(0.1), np.log(0.1), np.log(1.0),np.log(0.2)]),modelBias), axis=0)
    _, grads_computed = log_obsZs_giv_par_with_grad(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp,gp_deltas_modelOut = True)
    grads_approx = gradsApprox(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, elev_fp ,withPrior= False, gp_deltas_modelOut = True)
...
if __name__ == '__main__':
    ...
    output_folder  = output_folder + 'poly_deg' + str(args.poly_deg) 
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    output_folder += '/'
    ...
    if args.crossValFlag:    
        ...
        mu, cov = optimise(X_train, y_train, X_tildZs, y_tildZs, elev_fp, args.withPrior, args.poly_deg, args.gpdtsMo, args.useGradsFlag, args.repeat, args.SEED, args.numMo)
