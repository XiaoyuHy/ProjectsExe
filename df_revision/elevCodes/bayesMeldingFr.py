...
def test_fun(useCluster, SEED):
	...
	# _, grads = log_obsZs_giv_par_with_grad(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = True, withPrior= False, \
	...
	# print(res)
	# log_pos, _ = log_obsZs_giv_par_with_grad(initial_theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, withPrior= False, \
	# a_bias_poly_deg = 5, rbf = True, OMEGA = 1e-6)
	# print(log_pos)
...
def log_obsZs_giv_par_with_grad(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = True, withPrior= False, \
	...
	print(tmp.shape, X_tildZs_mean_extend.shape)
...
def optimise(X_hatZs, y_hatZs, X_tildZs, y_tildZs, withPrior, poly_deg, gpdtsMo=False, useGradsFlag = False, repeat=3, seed =188, numMo =50, rounds =1, method='L-BFGS-B', rbf=True, OMEGA = 1e-6, \
	...
	if numMo == numMo:
		...
		for round in range(rounds):
			res_tmp = optim_RndStart_BFGS(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat, seed, numMo, rounds)
		...
		for round in range(rounds):
			if round ==0:
				res_tmp = optim_NotRndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs, gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat,seed, numMo, rounds)
				# print 'res_tmp is ' + str(res_tmp)
				res.append(res_tmp)
			else:
				res_tmp = optim_RndStart(X_hatZs, y_hatZs, X_tildZs, y_tildZs,gpdtsMo, useGradsFlag, withPrior, poly_deg, num_par, method, bounds, repeat,seed, numMo, rounds)
				# print 'res_tmp is ' + str(res_tmp)
				res.append(res_tmp)
		res = np.array(res)
		res = res.T
...
def gradsApprox(theta, X_hatZs, y_hatZs, X_tildZs, y_tildZs, gp_deltas_modelOut = False,  withPrior= False,  \
	...
	a_bias_poly_deg = a_bias_poly_deg) # Step 3
...
def check_grads():
	...
	areal_hatZs = pickle.load(areal_hatZs_in)
	# modelBias = np.array([0.6, 0.1, 2.5, 2.1, 2.6, 5.47022754 , 5.22854712, 2.5])
	modelBias = np.array([0.6, 0.1, 5.47022754 , 5.22854712, 2.5])
	initial_theta=np.concatenate((np.array([np.log(1.5), np.log(0.1), np.log(0.1), np.log(1.0),np.log(0.2)]),modelBias), axis=0)
